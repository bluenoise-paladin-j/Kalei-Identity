...
AFRAME.registerComponent('butterfly-keyboard', {
  init: function () {
    ...
    this._resetTimer = null;           // v11: accept()'s fallback, cancelled by reset() -- see accept()
    this.keys = [];
    this.buttons = [];
    this.nameSprites = [];
  ...
  buildPanel: function () {
    ...
    var specs = [
      ...
      //  v10.5: each carries its own glyph -- enter for accept, the
      //  delete mark for delete -- pinned at its own small spin so it
      //  reads as sitting IN the flower rather than stamped level on
      //  top of it, the same "neither of them square to you" idea the
      //  tilt/cant above already apply to the shape itself.
      { id: 'accept', hue: 142, seed: 0.9, x:  CFG.ctlGap * 0.56, dy:  0.050, k: 1.26,
        tilt: -0.33, cant:  0.24, lit: { off: 41, idle: 38, hot: 31 },
        icon: 'accept', iconSpin: -0.12 },
      { id: 'delete', hue: 356, seed: 2.7, x: -CFG.ctlGap * 0.44, dy: -0.075, k: 1.02,
        tilt:  0.50, cant: -0.20, lit: { off: 53, idle: 49, hot: 41 },
        icon: 'delete', iconSpin: 0.15 }
    ];
    for (var i = 0; i < specs.length; i++) {
      var sp = specs[i];
      var b = UI.blob(sp.seed, CFG.blobW * sp.k, CFG.blobH * sp.k,
        { tex: UI.icon(sp.icon), spin: sp.iconSpin });
  ...
  accept: function () {
    ...
    //  its queue, or a name the seam refuses.
    //
    //  v11 -- IT IS CANCELLED BY reset(), and that is not tidiness. The
    //  fallback is twenty seconds; the normal path resets at about nine
    //  (the reveal's seven beats, then the caught name's two-second fall).
    //  Left armed, it fired ELEVEN SECONDS INTO THE NEXT NAME and wiped
    //  it -- `reset()` is idempotent but it is not harmless once somebody
    //  has started spelling again, which after a reveal they very often
    //  have. Being idempotent was never the property this needed; being
    //  cancelled is. Verified in the running scene by shortening the delay
    //  and spelling through it.
    if (this._resetTimer) { clearTimeout(this._resetTimer); }
    this._resetTimer = setTimeout(function () {
      self._resetTimer = null;
      self.reset();
    }, CFG.acceptResetDelay);
  },

  reset: function () {
    //  ...and disarm the accept fallback, whoever got here first. See
    //  accept(): a timer still running is a timer that will clear the
    //  NEXT visitor's name out from under them.
