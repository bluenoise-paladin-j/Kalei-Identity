...
var Wings = (function () {
  ...
  function keyFor(dials) {
    //  v8.7: the fit knobs change the pixels, so they have to be in the
    //  key or a tuning change would be served a stale texture.
    function fk(key, cfgKey) {
      if (opts[key] !== undefined) { return opts[key]; }
      if (typeof CFG !== 'undefined' && CFG[cfgKey] !== undefined) { return CFG[cfgKey]; }
      return WingGen.DEFAULTS[key];
    }
    return dials.map(function (d) { return d.toFixed(6); }).join(',') +
           ...
           '|' + fk('fitFill', 'wingFitFill') + ',' + fk('fitSeam', 'wingFitSeam') +
           ',' + fk('fitGain', 'wingFitGain') + ',' + fk('fitMargin', 'wingFitMargin');
  }
